...
Remember: The fewer questions you ask, the higher the score!`,d=[{name:"キャラクター",args:{action:"start",category:"character"}},{name:"有名人",args:{action:"start",category:"person"}},{name:"動物",args:{action:"start",category:"animal"}},{name:"もの",args:{action:"start",category:"object"}},{name:"場所",args:{action:"start",category:"place"}}],f=t=>{switch(t){case"character":return"キャラクター";case"person":return"有名人";case"animal":return"動物";case"object":return"もの";case"place":return"場所"}},A=t=>({phase:"questioning",category:t,questionCount:0,maxQuestions:20,qaHistory:[],message:`🔮 ${f(t)}を当てます！何か1つ思い浮かべてください。準備ができたら教えてね！`}),T=(t,g)=>g?t<=5?100:t<=10?80:t<=15?60:t<=20?40:20:0,b=async(t,g)=>{const{action:l,category:w,answer:i,guess:u,wasCorrect:n,actualAnswer:h}=g,s=t.currentResult?.data?.state;try{let e,r,a;switch(l){case"start":{if(!w)throw new Error("Category is required to start the game");e=A(w),r=`Game started! Category: ${f(w)}. Ask your first yes/no question to start narrowing down. Use broad questions first (e.g., "Is it a real person?" or "Is it from Japan?").
...
- "わからない" → answer="unknown"`,a=["Ask a yes/no question, then call with action='answer' when user responds"];break}case"answer":{if(!s)throw new Error("No active game. Start a new game first.");if(!i)throw new Error("Answer is required");e={...s,questionCount:s.questionCount+1,qaHistory:s.currentQuestion?[...s.qaHistory,{question:s.currentQuestion,answer:i}]:s.qaHistory,currentQuestion:void 0};const o={yes:"はい",no:"いいえ",probably_yes:"たぶんはい",probably_no:"たぶんいいえ",unknown:"わからない"}[i];if(e.message=`📝 回答: ${o} (質問 ${e.questionCount}/${e.maxQuestions})`,e.questionCount>=e.maxQuestions)e.phase="guessing",r=`You've reached the maximum ${e.maxQuestions} questions! Based on all the answers, make your best guess now using action='guess'.`,a=["guess"];else{const q=e.qaHistory.map((y,k)=>`Q${k+1}: ${y.question} → ${y.answer}`).join(`
...
2. If confident, make a guess with action='guess'`,a=["Ask another question","guess"]}break}case"guess":{if(!s)throw new Error("No active game. Start a new game first.");if(!u)throw new Error("Guess is required");e={...s,phase:"guessing",guess:u,message:`🎯 私の予想は... 「${u}」です！

当たっていますか？`},r=`You guessed "${u}". Wait for the user to confirm if it's correct. When they respond, call action='reveal' with wasCorrect=true/false. If wrong, include actualAnswer with what they were thinking of.`,a=["reveal (with wasCorrect and optionally actualAnswer)"];break}case"reveal":{if(!s)throw new Error("No active game. Start a new game first.");if(n===void 0)throw new Error("wasCorrect is required");const o=T(s.questionCount,n);e={...s,phase:"result",isCorrect:n,score:o,message:n?`🎉 正解！ ${s.questionCount}問で当てました！
⭐ スコア: ${o}点`:`😅 残念！正解は「${h||"???"}」でした。
次は頑張ります！`},r=n?`Congratulations! You guessed correctly in ${s.questionCount} questions! Score: ${o}. Celebrate with the user and offer to play again.`:`Your guess was wrong. The answer was "${h}". Acknowledge this gracefully and offer to play again.`,a=["start (to play again)"];break}default:throw new Error(`Unknown action: ${l}`)}return{toolName:c.name,data:{state:e,message:e.message},jsonData:{state:e,availableActions:a,instructions:r},message:e.message,instructions:r,updating:l!=="start",instructionsRequired:!0}}catch(e){return console.error("Akinator game error",e),{toolName:c.name,message:`エラー: ${e instanceof Error?e.message:"Unknown error"}`,instructions:"An error occurred. Ask the user if they want to start a new game."}}},$={toolDefinition:c,execute:b,generatingMessage:"🔮 考え中...",isEnabled:()=>!0,samples:d,systemPrompt:p};exports.SAMPLES=d;exports.SYSTEM_PROMPT=p;exports.TOOL_DEFINITION=c;exports.TOOL_NAME=m;exports.executeAkinator=b;exports.pluginCore=$;
